...
resource "azurerm_public_ip" "bas_pip" {
  name                = "pip-${var.AZURE_SHORT}-${var.loc}-${terraform.workspace}"
...
resource "azurerm_bastion_host" "bas_host" {
  name                = "bas-${var.AZURE_SHORT}-${var.loc}-${terraform.workspace}"
  location            = azurerm_resource_group.net_rg.location
  resource_group_name = azurerm_resource_group.net_rg.name
...
resource "azurerm_network_security_group" "bas_nsg" {
  name                = "nsg-bas-${var.AZURE_SHORT}-${var.loc}-${terraform.workspace}"
  location            = azurerm_resource_group.net_rg.location
  resource_group_name = azurerm_resource_group.net_rg.name
  tags                = local.tags
}
